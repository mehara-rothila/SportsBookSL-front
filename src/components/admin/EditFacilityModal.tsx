...
export default function EditFacilityModal({ isOpen, onClose, facility, onSave }: EditFacilityModalProps) {
    ...
    if (facility && isOpen) {
      ...
      const initialHours = daysOfWeek.map(day => {
          ...
          return {
              ...
              isClosed: !existing || (!existing.open && !existing.close) // Assume closed if no entry exists or if entry exists but has no time
          };
      });
  ...
  const handleImageChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Store new files and set clearing flag
  ...
  const removeNewImagePreview = (index: number) => {
      ...
      // If all new previews removed, revert to existing images
  ...
  const handleSubmit = async () => {
    ...
    if (!formData.name || !formData.location || !formData.address || !formData.description || 
        !formData.sportTypes || !formData.pricePerHour || 
        formData.pricePerHourValue === undefined || formData.pricePerHourValue < 0) {
      setError("Please fill in all required fields (*).");
      return;
    }

    // Validate images
    if (clearExistingImages && newImageFiles.length === 0 && existingImagePaths.length > 0) {
    ...
    // Prepare only changed data compared to the original facility
    const dataToUpdate: Partial<facilityService.FacilityFormData> = {};
    
    // Helper function to check if values are different
    const isDifferent = (currentValue: any, originalValue: any): boolean => {
    ...
    (Object.keys(formData) as Array<keyof typeof formData>).forEach(key => {
        ...
        else if (key === 'isActive' && facility[key as keyof FacilityData] === undefined) originalValue = true;
        else originalValue = facility[key as keyof FacilityData];

        // If value has changed, add to update object
        if (isDifferent(formData[key], originalValue)) {
    ...
    // Compare current hours with original hours
    const currentHoursString = JSON.stringify(operatingHoursToSave.sort((a, b) => 
        daysOfWeek.indexOf(a.day) - daysOfWeek.indexOf(b.day)));
        
    const originalHours = (facility.operatingHours || [])
        .map(({ day, open, close }) => ({ day, open, close }));
        
    const originalHoursString = JSON.stringify(originalHours.sort((a, b) => 
        daysOfWeek.indexOf(a.day) - daysOfWeek.indexOf(b.day)));
    ...
    try {
      // Add operating hours to data if changed
  ...
  // Render null if modal is closed or no facility data
                ...
                <form onSubmit={(e) => { e.preventDefault(); handleSubmit(); }}>
                  <div className="space-y-6 max-h-[70vh] overflow-y-auto pr-3 custom-scrollbar">
                    ...
                    <fieldset className="border p-4 rounded-md border-white/20 bg-white/5 backdrop-blur-sm">
                        ...
                        <div className="grid grid-cols-1 gap-y-4 gap-x-4 sm:grid-cols-6 pt-2">
                            <div className="sm:col-span-6"> 
                              <label htmlFor="fac_name_edit" className="form-label-admin"> 
                            ...
                            <div className="sm:col-span-3"> 
                              <label htmlFor="fac_location_edit" className="form-label-admin"> 
                                Location (City/Area) <span className="text-red-400">*</span> 
                              </label> 
                              <input 
                                type="text" 
                                name="location" 
                                id="fac_location_edit" 
                            ...
                            <div className="sm:col-span-3"> 
                              <label htmlFor="fac_address_edit" className="form-label-admin"> 
                            ...
                            <div className="sm:col-span-6"> 
                              <label htmlFor="fac_description_edit" className="form-label-admin"> 
                                Short Description <span className="text-red-400">*</span> 
                              </label> 
                              <textarea 
                                name="description" 
                                id="fac_description_edit" 
                            ...
                            <div className="sm:col-span-6"> 
                              <label htmlFor="fac_longDescription_edit" className="form-label-admin"> 
                                Long Description 
                              </label> 
                              <textarea 
                                name="longDescription" 
                                id="fac_longDescription_edit" 
                    ...
                    <fieldset className="border p-4 rounded-md border-white/20 bg-white/5 backdrop-blur-sm">
                        ...
                        <div className="grid grid-cols-1 gap-y-4 gap-x-4 sm:grid-cols-6 pt-2">
                            <div className="sm:col-span-6"> 
                              <label htmlFor="fac_sportTypes_edit" className="form-label-admin"> 
                                Sport Types <span className="text-red-400">*</span> 
                              </label> 
                              <input 
                                type="text" 
                                name="sportTypes" 
                                id="fac_sportTypes_edit" 
                                value={formData.sportTypes} 
                                onChange={handleChange} 
                                required 
                                className="input-field-admin"
                              /> 
                              <p className="input-help-text-admin">Comma-separated</p> 
                            </div>
                            <div className="sm:col-span-3"> 
                              <label htmlFor="fac_pricePerHour_edit" className="form-label-admin"> 
                                Price Display <span className="text-red-400">*</span> 
                              </label> 
                              <input 
                                type="text" 
                                name="pricePerHour" 
                                id="fac_pricePerHour_edit" 
                            ...
                            <div className="sm:col-span-3"> 
                              <label htmlFor="fac_pricePerHourValue_edit" className="form-label-admin"> 
                                Price Value (Number) <span className="text-red-400">*</span> 
                              </label> 
                              <input 
                                type="number" 
                            ...
                            <div className="sm:col-span-3"> 
                              <label htmlFor="fac_pricePerDay_edit" className="form-label-admin"> 
                                Price Per Day 
                              </label> 
                              <input 
                                type="number" 
                                name="pricePerDay" 
                                id="fac_pricePerDay_edit" 
                    ...
                    <fieldset className="border p-4 rounded-md border-white/20 bg-white/5 backdrop-blur-sm">
                        ...
                        <div className="grid grid-cols-1 gap-y-4 gap-x-4 sm:grid-cols-6 pt-2">
                            <div className="sm:col-span-6"> 
                              <label htmlFor="fac_amenities_edit" className="form-label-admin"> 
                                Amenities 
                              </label> 
                              <input 
                                type="text" 
                                name="amenities" 
                                id="fac_amenities_edit" 
                            ...
                            <div className="sm:col-span-6"> 
                              <label className="form-label-admin"> Flags </label> 
                    ...
                    <fieldset className="border p-4 rounded-md border-white/20 bg-white/5 backdrop-blur-sm">
                        ...
                        <div className="grid grid-cols-1 gap-y-4 gap-x-4 sm:grid-cols-6 pt-2">
                            <div className="sm:col-span-3"> 
                              <label htmlFor="fac_contactPhone_edit" className="form-label-admin"> 
                                Contact Phone 
                              </label> 
                              <input 
                                type="tel" 
                                name="contactPhone" 
                                id="fac_contactPhone_edit" 
                            ...
                            <div className="sm:col-span-3"> 
                              <label htmlFor="fac_contactEmail_edit" className="form-label-admin"> 
                                Contact Email 
                              </label> 
                              <input 
                                type="email" 
                                name="contactEmail" 
                                id="fac_contactEmail_edit" 
                            ...
                            <div className="sm:col-span-6"> 
                              <label htmlFor="fac_contactWebsite_edit" className="form-label-admin"> 
                                Website URL 
                              </label> 
                              <input 
                                type="url" 
                                name="contactWebsite" 
                                id="fac_contactWebsite_edit" 
                            ...
                            <div className="sm:col-span-3"> 
                              <label htmlFor="fac_mapLat_edit" className="form-label-admin"> 
                                Map Latitude 
                              </label> 
                              <input 
                                type="number" 
                            ...
                            <div className="sm:col-span-3"> 
                              <label htmlFor="fac_mapLng_edit" className="form-label-admin"> 
                                Map Longitude 
                              </label> 
                              <input 
                                type="number" 
                    ...
                    <fieldset className="border p-4 rounded-md border-white/20 bg-white/5 backdrop-blur-sm">
                        ...
                        <div className="pt-2">
                            <label htmlFor="fac_images_edit" className="form-label-admin"> 
                            ...
                            <p className="input-help-text-admin">
                              Selecting new images will <span className='font-medium'>replace all</span> existing images. Max 5 files.
                            </p>
                            ...
                            {newImageFiles.length === 0 && existingImagePaths.length > 0 && (
                                <div className="mt-4 flex items-center">
                                    <input 
                                      ...
                                      className="h-4 w-4 rounded border-white/30 bg-white/5 text-emerald-600 focus:ring-emerald-500"
                                    />
                                    <label htmlFor="clearImages_edit" className="ml-2 text-sm text-red-300">
                                      Clear all existing images on save?
                                    </label>
                                </div>
                            )}
                        </div>
