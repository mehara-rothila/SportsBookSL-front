...
export default function FinancialAidDetailModal({
  ...
  const formatDate = (dateStr: string | Date | undefined, formatString = 'PPP') => { if (!dateStr) return 'N/A'; try { return format(parseISO(String(dateStr)), formatString); } catch (e) { return 'Invalid Date'; } };
  ...
  const getStatusPill = (status: ApplicationDetails['status'] | undefined) => { switch (status) { case 'pending': return <span className="chip-yellow">Pending</span>; case 'approved': return <span className="chip-green">Approved</span>; case 'rejected': return <span className="chip-red">Rejected</span>; case 'needs_info': return <span className="chip-blue">Needs Info</span>; default: return <span className="chip-gray">Unknown</span>; }};
  ...
  const DetailItem = ({ label, value }: { label: string; value: React.ReactNode }) => ( value ? <div className="flex"><strong className="w-28 font-medium text-gray-600 shrink-0">{label}:</strong><span className="break-words">{value}</span></div> : null );
                ...
                <div className="space-y-6">
                     <DetailSection title="Application Status" icon={InformationCircleIcon}>
                      ...
                      <DetailSection title="Personal Info Snapshot" icon={UserCircleIcon}>
                         <DetailItem label="Full Name" value={application.personalInfoSnapshot?.fullName}/>
                         <DetailItem label="Email" value={application.personalInfoSnapshot?.email}/>
                         <DetailItem label="Phone" value={application.personalInfoSnapshot?.phone}/>
                         <DetailItem label="DOB" value={formatDate(application.personalInfoSnapshot?.dateOfBirth)}/>
                         <DetailItem label="Address" value={`${application.personalInfoSnapshot?.address || ''}, ${application.personalInfoSnapshot?.city || ''}, ${application.personalInfoSnapshot?.postalCode || ''}`}/>
                     </DetailSection>
                    ...
                    <DetailSection title="Sports Background" icon={ClipboardDocumentListIcon}>
                        ...
                        <DetailItem label="Experience" value={application.sportsInfo?.yearsExperience ? `${application.sportsInfo.yearsExperience} years` : 'N/A'} />
...
// Chip styles (ensure these are in your global CSS)
/*
