...
interface Facility { _id: string; id?: string; name: string; location: string; address: string; description: string; longDescription?: string; sportTypes: string[]; amenities: string[]; pricePerHour: string; pricePerHourValue: number; pricePerDay?: number; rating: number; reviewCount: number; contactInfo: { phone?: string; email?: string; website?: string; }; images: string[]; operatingHours: OperatingHours[]; equipmentForRent: EquipmentItem[]; associatedCoaches: Coach[]; mapLocation?: { lat?: number; lng?: number; }; isNew?: boolean; isPremium?: boolean; isFeatured?: boolean; reviews?: Review[]; specialRates?: { name: string; description: string; rate: number; conditions: string }[]; rules?: string[]; }
