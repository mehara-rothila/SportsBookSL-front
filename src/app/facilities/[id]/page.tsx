...
// ReviewList component might not be needed if rendered inline
...
const FALLBACK_AVATAR = '/images/default-avatar.png'; // Ensure this exists

// --- Image URL Helper ---
const getImageUrl = (imagePath: string | undefined, fallback: string): string => {
    if (!imagePath) {
        return fallback;
    }
    // Check if the path already contains http/https (e.g., from CDN or absolute path stored)
    ...
    // Combine base URL and formatted path
    const fullUrl = `${baseUrl}${formattedPath}`;

    // Basic check for potential double slashes (though the above logic should prevent it)
...
export default function FacilityDetailPage() {
    ...
    // Initialize mainImage state with the fallback
    const [mainImage, setMainImage] = useState<string>(FALLBACK_IMAGE);
    ...
    useEffect(() => {
        ...
        const fetchFacility = async () => {
            ...
            try {
                ...
                setFacility(data);
                setReviews(data.reviews || []); // Assuming reviews are included in the initial fetch
                const initialReviewLimit = 5; // Set your desired limit
                setHasMoreReviews((data.reviews?.length || 0) >= initialReviewLimit && (data.reviewCount || 0) > initialReviewLimit);
                setTotalReviewPages(Math.ceil((data.reviewCount || 0) / initialReviewLimit));
                setReviewPage(1);

                // Set the main image using the helper function
                setMainImage(getImageUrl(data.images?.[0], FALLBACK_IMAGE));

            } catch (err: any) {
                console.error('Error fetching facility data:', err);
                setError(typeof err === 'string' ? err : err.message || 'Failed to load facility details.');
                setFacility(null);
                setMainImage(FALLBACK_IMAGE); // Ensure fallback on error
            } finally {
                setLoading(false);
            }
        };
        fetchFacility();
    }, [id]); // Dependency: only the facility ID
    ...
    const fetchAvailability = useCallback(async (monthDate: Date) => {
        ...
        try {
            ...
            // Ensure date parsing handles timezone correctly to avoid off-by-one day issues
            const parsedData = (Array.isArray(data) ? data : []).map(item => ({
                ...item,
                // Parse as UTC date string then treat as local date object
            ...
            // Optionally set an error state for availability
            setAvailabilityData([]);
        } finally {
    ...
    }, [currentAvailabilityMonth, fetchAvailability]); // Re-fetch when month changes
    ...
    const fetchMoreReviews = async () => {
        ...
        const reviewLimit = 5; // Match initial limit or desired page size
        try {
            // Assuming getFacilityReviews fetches paginated reviews
            const data = await facilityService.getFacilityReviews(id, nextPage, reviewLimit);
            const newReviews = (data.reviews || []) as unknown as Review[]; // Ensure type safety
            setReviews(prev => [...prev, ...newReviews]);
            setReviewPage(nextPage);
            setTotalReviewPages(data.pages || 1); // Assuming API returns total pages
    ...
    const handleReviewSubmitSuccess = (newReviewData: Review) => {
        ...
        // Construct avatar URL correctly using the helper
        const userAvatarUrl = getImageUrl(
            newReviewData.user?.avatar || currentUser?.avatar,
            FALLBACK_AVATAR
        );

        const reviewWithUser: Review = {
            ...newReviewData,
            _id: newReviewData._id || `temp-${Date.now()}`, // Use ID from response or generate temporary
            user: {
                _id: newReviewData.user?._id || currentUser?._id || 'temp-user-id',
                name: newReviewData.user?.name || currentUser?.name || 'You',
                avatar: userAvatarUrl // Use the correctly constructed URL
            },
            // Ensure date exists, fallback to now if needed
            reviewDate: newReviewData.reviewDate || newReviewData.createdAt || new Date().toISOString()
        };
        ...
        // Update facility review count and potentially rating (refetch might be better for rating)
        if (facility && id) {
            setFacility(prevFac => {
                if (!prevFac) return null;
                const newReviewCount = (prevFac.reviewCount || 0) + 1;
                // Simple count update. For rating, refetching is safer.
                return {
                    ...prevFac,
                    reviewCount: newReviewCount,
                };
            });
            // Optionally refetch the facility data to get updated average rating
            // This ensures rating is accurate after new submission
            facilityService.getFacilityById(id).then(data => {
                setFacility(data);
                 // Update main image again in case it changed somehow (unlikely but safe)
                setMainImage(getImageUrl(data.images?.[0], FALLBACK_IMAGE));
            }).catch(console.error);
        }
    };
    ...
    const calculateTotal = useMemo(() => {
        if (!facility?.pricePerHourValue) return 0;
        // Assume 2-hour slot based on previous logic. Make dynamic if needed.
        const durationHours = 2;
        let total = selectedTimeSlot ? (facility.pricePerHourValue * durationHours) : 0;
        Object.entries(equipmentSelection).forEach(([name, quantity]) => {
            const equipment = facility.equipmentForRent?.find(e => e.name === name);
            if (equipment && quantity > 0) {
                total += (equipment.pricePerHour || 0) * quantity * durationHours;
            }
        });
        return total;
    }, [facility, selectedTimeSlot, equipmentSelection]);

    // Use getImageUrl when setting the lightbox image state
    const openLightbox = (imagePath: string | undefined) => {
        if (!imagePath) return; // Don't open if path is missing
        setLightboxImage(getImageUrl(imagePath, FALLBACK_IMAGE));
        setLightboxOpen(true);
    };

    const handleProceedToBooking = () => {
        if (!facility || !id) return; // Check facility and id exist
        if (selectedDate && selectedTimeSlot) {
            const bookingUrl = `/facilities/${id}/book`; // Use id directly
            const queryParams: Record<string, string> = {
                date: formatDateFns(selectedDate, 'yyyy-MM-dd'),
                time: selectedTimeSlot,
            };
            // Filter only equipment with quantity > 0 for the query param
            const selectedEquipmentForQuery = Object.entries(equipmentSelection)
            ...
            if (Object.keys(selectedEquipmentForQuery).length > 0) {
                queryParams.equipment = JSON.stringify(selectedEquipmentForQuery);
            }
            const urlWithParams = `${bookingUrl}?${new URLSearchParams(queryParams).toString()}`;
            router.push(urlWithParams);
        } else {
            alert("Please select a date and time slot first."); // Provide user feedback
        }
    };
    ...
    if (error) {
        return (
            <div className="flex flex-col justify-center items-center min-h-screen bg-gradient-to-br from-emerald-800 via-green-800 to-emerald-900 text-white p-8">
                 <h2 className="text-2xl font-bold mb-4 text-red-300">Error Loading Facility</h2>
                <p className="mb-6 text-center text-red-200">{error}</p>
    ...
    if (!facility) { // Should ideally be caught by error state, but good failsafe
        return (
            <div className="flex flex-col justify-center items-center min-h-screen bg-gradient-to-br from-emerald-800 via-green-800 to-emerald-900 p-8">
                ...
                <p className="text-emerald-100 mb-4 text-center">We couldn't find the facility details (ID: {id}). It might have been removed.</p>
    ...
    // No need for mainImageUrl variable, use mainImage state directly
    // const mainImageUrl = mainImage; // Removed

    // --- Main Return JSX ---
    return (
        <div className="min-h-screen relative overflow-hidden">
            {/* Background Animations */}
            {/* ... (background animation divs remain the same) ... */}
            ...
            <div className="relative">
                 {/* --- REFACTORED HERO IMAGE --- */}
                <div className="h-[60vh] md:h-[70vh] w-full relative group">
                    {/* Image Container */}
                    <div className="absolute inset-0 overflow-hidden">
                         <img
                            src={mainImage} // Use state variable which holds full URL or fallback
                            alt={`${facility.name} main view`}
                            className="absolute inset-0 w-full h-full object-cover object-center transition-transform duration-1000 ease-in-out group-hover:scale-110 animate-ken-burns"
                            // Standard onError for img tag
                            onError={(e) => {
                                const imgElement = e.target as HTMLImageElement;
                                if (imgElement.src !== FALLBACK_IMAGE) {
                                    imgElement.src = FALLBACK_IMAGE;
                                    // Update state to ensure consistency if needed, though direct src change often suffices visually
                    ...
                    <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
                        <div className="flex space-x-4 overflow-x-auto pb-2 custom-scrollbar">
                            {facility.images?.map((imagePath, index) => {
                                // Use helper for thumbnail src
                                const thumbUrl = getImageUrl(imagePath, FALLBACK_IMAGE);
                                // Use helper for setting main image on click
                                const mainOnClickUrl = getImageUrl(imagePath, FALLBACK_IMAGE);
                                return (
                                    <button
                                        key={index}
                                        className={`flex-shrink-0 h-20 w-32 md:h-24 md:w-40 rounded-lg overflow-hidden transition-all duration-300 transform ${mainImage === mainOnClickUrl ? 'border-4 border-emerald-500 scale-110 shadow-2xl' : 'border-2 border-transparent hover:border-emerald-400 hover:scale-105'}`}
                                        // Set mainImage state with the correctly constructed full URL
                                        onClick={() => setMainImage(mainOnClickUrl)}
                                    >
                                        <div className="h-full w-full overflow-hidden">
                                            <img
                                                src={thumbUrl} // Use the constructed thumbnail URL
                            ...
                            {/* Button to open lightbox uses the first image path */}
                            {facility.images && facility.images.length > 0 && (
                                <button
                                    ...
                                    onClick={() => openLightbox(facility.images?.[0])} // Pass the relative path
                                >
                    ...
                    <div className="lg:col-span-2">
                        <Tab.Group>
                            ...
                            <Tab.Panels className="mt-2">
                                ...
                                {/* Equipment Panel */}
                                <Tab.Panel className="rounded-2xl bg-white/20 backdrop-blur-sm p-6 md:p-8 shadow-xl border border-white/20">
                                     {/* ... Equipment content remains the same ... */}
                                     <EquipmentRental
                                ...
                                <Tab.Panel className="rounded-2xl bg-white/20 backdrop-blur-sm p-6 md:p-8 shadow-xl border border-white/20">
                                    ...
                                    {facility.associatedCoaches && facility.associatedCoaches.length > 0 ? (
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                            {facility.associatedCoaches.map((coach) => {
                                                // Use getImageUrl for coach profile picture
                                                const coachImageUrl = getImageUrl(coach.profileImage, FALLBACK_AVATAR);
                                                return (
                                                            ...
                                                            <div className="flex flex-col sm:flex-row h-full">
                                                                <div className="sm:w-2/5 h-48 sm:h-auto overflow-hidden relative">
                                                                    <img
                                                                        ...
                                                                        src={coachImageUrl} // Use constructed URL
                                                                        alt={coach.name}
                                                                        loading="lazy"
                                                                        onError={(e) => {
                                                                            const imgElement = e.target as HTMLImageElement;
                                                                            if (imgElement.src !== FALLBACK_AVATAR) {
                                                                                imgElement.src = FALLBACK_AVATAR;
                                                                            }
                                                                        }}
                                                                    />
                                                                </div>
                                                                <div className="p-5 flex flex-col justify-between sm:w-3/5 relative">
                                                                    <div>
                                                                        <div className="flex justify-between items-start mb-1">
                                                                            ...
                                                                            <div className="flex items-center bg-amber-400/20 rounded-full px-2 py-1"><StarIcon className="w-4 h-4 mr-1 text-amber-400"/><span className="font-bold text-amber-300">{coach.rating?.toFixed(1) ?? 'N/A'}</span></div>
                                                                        </div>
                                                                        <div><p className="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-white/10 text-white mb-2 border border-white/20 shadow-sm"><AcademicCapIcon className="w-3.5 h-3.5 mr-1"/>{coach.specialization}</p></div>
                                                                        <div className="flex items-center text-lg font-bold text-emerald-300 mb-3"><CurrencyDollarIcon className="w-5 h-5 mr-1 text-emerald-400"/>{formatCurrency(coach.hourlyRate)}/hr</div>
                                                                    </div>
                                    ...
                                    <div className="mt-10 text-center">
                                        <Link href="/trainers">
                                            <Button variant="gradient"> View all trainers <ArrowRightIcon className="ml-2 h-5 w-5"/></Button>
                                        </Link>
                                    </div>
                                </Tab.Panel>
                                        ...
                                        {reviews.length === 0 ? (
                                            ...
                                            <div className="space-y-6">
                                                {reviews.map((review, index) => {
                                                    // Use getImageUrl for user avatar
                                                    const avatarUrl = getImageUrl(review.user?.avatar, FALLBACK_AVATAR);
                                                    return (
                                                        <div
                                                            ...
                                                            <div className="flex items-start">
                                                                <div className="flex-shrink-0">
                                                                    <img
                                                                        ...
                                                                        src={avatarUrl} // Use constructed URL
                                                                        ...
                                                                        <div>
                                                                            ...
                                                                            <div className="flex items-center mt-1">
                                                                                ...
                                                                                <time className="ml-2 text-sm text-white/60" dateTime={review.reviewDate || review.createdAt}>
                                                                                    {new Date(review.reviewDate || review.createdAt || Date.now()).toLocaleDateString('en-US', {
                                                                                        year: 'numeric',
                                                                                        month: 'long',
                                                                                        day: 'numeric'
                                                                                    })}
                                ...
                                <div className="bg-gradient-to-br from-emerald-900/80 to-green-900/80 rounded-2xl overflow-hidden backdrop-blur-sm">
                                    ...
                                    <div className="p-6">
                                        <div className="space-y-6">
                                            {/* Date Selection */}
                                            <div>
                                                <label htmlFor="booking-calendar" className="block text-sm font-bold text-white mb-2 flex items-center"><CalendarDaysIcon className="w-5 h-5 mr-1.5 text-emerald-400"/> Select a Date</label>
                                                <BookingCalendar
                                                    id="booking-calendar" // Add id for label association
                                                ...
                                                />
                                                {loadingAvailability && <p className="text-center text-sm text-emerald-200/70 mt-2 animate-pulse">Loading availability...</p>}
                                            </div>
                                                ...
                                                <div className="animate-fade-in">
                                                    ...
                                                    <div className="space-y-3 max-h-48 overflow-y-auto pr-2 bg-gradient-to-r from-emerald-900/50 to-green-900/40 rounded-xl p-4 shadow-inner border border-white/20 custom-scrollbar relative">
                                                        ...
                                                        {facility.equipmentForRent.map((equipment) => (
                                                            <div key={equipment._id || equipment.name} className="flex items-center justify-between bg-white/10 backdrop-blur-sm p-2.5 rounded-lg border border-white/20 shadow-sm hover:shadow-md hover:bg-white/15 transition-all duration-200 transform hover:scale-[1.02]">
                                                                <div>
                                                                ...
                                                                <div className="flex items-center">
                                                                    ...
                                                                    <select
                                                                        ...
                                                                        className="rounded-lg border-emerald-800 text-sm py-1.5 shadow-sm focus:ring-emerald-500 focus:border-emerald-500 bg-emerald-900/70 text-white appearance-none"
                                                                        ...
                                                                        style={{ backgroundImage: `url("data:image/svg+xml,%3csvg xmlns='http://www.w3.org/2000/svg' fill='none' viewBox='0 0 20 20'%3e%3cpath stroke='%23a7f3d0' stroke-linecap='round' stroke-linejoin='round' stroke-width='1.5' d='M6 8l4 4 4-4'/%3e%3c/svg%3e")`, backgroundRepeat: 'no-repeat', backgroundPosition: 'right 0.5rem center', backgroundSize: '1em 1em', paddingRight: '2rem' }}
                                                                    >
                                                                        {[...Array(Math.max(0, equipment.available) + 1).keys()].map((num) => (
                                                ...
                                                <div className="border-t-2 border-white/10 pt-5 mt-6 animate-fade-in space-y-3">
                                                    ...
                                                    <div className="bg-gradient-to-r from-emerald-900/50 to-green-900/40 rounded-xl p-4 shadow-inner border border-white/20">
                                                        <div className="flex justify-between text-sm border-b border-white/10 pb-2 mb-2">
                                                            <span className="text-gray-300">Facility Fee ({selectedTimeSlot})</span>
                                                            <span className="text-white font-medium">{formatCurrency(facility.pricePerHourValue ? facility.pricePerHourValue * 2 : 0)}</span>
                                                        </div>
                                                        {Object.entries(equipmentSelection)
                                                            .filter(([_, quantity]) => quantity > 0) // Only show items with quantity > 0
                                                            .map(([name, quantity]) => {
                                                                const equipment = facility.equipmentForRent?.find(e => e.name === name);
                                                                const itemCost = equipment ? (equipment.pricePerHour || 0) * quantity * 2 : 0;
                                                                if (equipment) {
                                                                    return (
                                                                        <div key={name} className="flex justify-between text-sm border-b border-white/10 pb-2 mb-2">
                                                                            <span className="text-gray-300">{name} (x{quantity})</span>
                                                                            <span className="text-white font-medium">{formatCurrency(itemCost)}</span>
                                                                        </div>
                                                                     );
                                                                }
                                                                return null;
                                                        ...
                                                        <div className="flex justify-between font-bold text-lg mt-4 pt-1">
                                                            ...
                                                            <span className="text-transparent bg-clip-text bg-gradient-to-r from-emerald-300 to-green-200">{formatCurrency(calculateTotal)}</span>
                                                        </div>
                                                    </div>
                                                </div>
                                            )}
                                            ...
                                            <Button
                                                ...
                                                aria-disabled={!(selectedDate && selectedTimeSlot)}
                                            >
                                                <CheckIconSolid className="w-5 h-5 mr-2"/> Proceed to Book
                                            </Button>
                                            <p className="text-xs text-emerald-200/70 text-center mt-3"> Confirm details & proceed to payment. </p>
                                        </div>
                                    </div>
                                </div>
                            </div>
            ...
            <section className="bg-gradient-to-r from-emerald-900/60 to-green-900/50 py-16 border-t border-white/10 relative backdrop-blur-sm">
                {/* ... Similar facilities content remains the same ... */}
                 <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 relative z-10">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
                        {/* NOTE: Using mock data & Unsplash. Replace with real data and use getImageUrl if applicable */}
                        {[
                             { _id: 'mock1', name: 'Green Valley Stadium', sportType: 'Cricket', rating: 4.6, reviews: 65, price: 7000, image: `https://source.unsplash.com/random/400x300?cricket,stadium&sig=1` },
                            { _id: 'mock2', name: 'Emerald Sports Complex', sportType: 'Cricket', rating: 4.5, reviews: 80, price: 7500, image: `https://source.unsplash.com/random/400x300?cricket,stadium&sig=2` },
                            { _id: 'mock3', name: 'Willow Cricket Ground', sportType: 'Cricket', rating: 4.4, reviews: 95, price: 8000, image: `https://source.unsplash.com/random/400x300?cricket,stadium&sig=3` },
                            { _id: 'mock4', name: 'Pavilion Training Center', sportType: 'Cricket', rating: 4.3, reviews: 110, price: 8500, image: `https://source.unsplash.com/random/400x300?cricket,stadium&sig=4` }
                        ].map((item) => (
                            <div key={item._id} className="group bg-gradient-to-br from-emerald-900/70 to-green-900/60 rounded-2xl shadow-lg overflow-hidden border border-white/10 hover:shadow-2xl transition-all duration-500 transform hover:-translate-y-2 backdrop-blur-sm">
                                <div className="aspect-w-16 aspect-h-9 relative overflow-hidden">
                                    <img
                                        src={item.image} // Using direct Unsplash URL here
                                        alt={item.name}
                                        className="w-full h-full object-cover transform group-hover:scale-110 transition-transform duration-700"
                                        loading="lazy"
                                        // No onError fallback needed for Unsplash in this example
                                    />
                                ...
                                <div className="p-5">
                                    ...
                                    <div className="flex justify-between items-center mt-4">
                                        ...
                                        {/* Link to a potential real facility page if ID was real */}
            ...
            {lightboxOpen && (
                <div className="fixed inset-0 z-[999] flex items-center justify-center bg-black/95 p-4 animate-fade-in backdrop-blur-sm" onClick={() => setLightboxOpen(false)}>
                    ...
                    <div onClick={(e) => e.stopPropagation()} className="relative max-w-5xl w-full group">
                        <img
                            src={lightboxImage} // Already set using getImageUrl
            ...
            <style jsx global>{`
                ...
                @keyframes umpire-movement { 0%, 100% { transform: translateY(0); } 50% { transform: translateY(-3px); } }
                .animate-umpire-movement { animation: umpire-movement 3s ease-in-out infinite; }
